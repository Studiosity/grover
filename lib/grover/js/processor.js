...
try {
  ...
  try {
    // Try load puppeteer-core (install sans Chromium)
    puppeteer = require(require.resolve('puppeteer-core', {paths: paths}));
  } catch (e) {
    // If core isn't available, try load puppeteer
    puppeteer = require(require.resolve('puppeteer', { paths: paths }));
  }
} catch (e) {
...
const _processPage = (async (convertAction, urlOrHtml, options) => {
  ...
  try {
    ...
    // Set the product if given
    const product = options.product; delete options.product;
    if (product) {
      launchParams.product = product;
    }
