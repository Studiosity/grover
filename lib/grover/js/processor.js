...
try {
  ...
  var puppeteer = require(require.resolve("puppeteer", {
...
const _processPage = async (convertAction, urlOrHtml, options) => {
  ...
  try {
    const remoteConnect = typeof options.browser_ws_endpoint === "string"

    // Launch or connect to the browser and create a page
    if (remoteConnect) {
      const connectParams = {
        browserWSEndpoint: options.browser_ws_endpoint
      }
      ...
      const launchParams = {
        args:
          process.env.GROVER_NO_SANDBOX === "true"
      ...
      if (typeof debug === "object" && !!debug) {
        if (debug.headless !== undefined) {
          launchParams.headless = debug.headless
        }
        if (debug.devtools !== undefined) {
          launchParams.devtools = debug.devtools
        }
      }
    ...
    const emulateMedia = options.emulateMedia
    ...
    const mediaFeatures = options.mediaFeatures
    delete options.mediaFeatures
    ...
    const visionDeficiency = options.visionDeficiency
    delete options.visionDeficiency
    if (visionDeficiency !== undefined) {
      page.emulateVisionDeficiency(type)
    }

    // Bypass CSP (content security policy), if provided
    const bypassCSP = options.bypassCSP
    delete options.bypassCSP
    ...
    const raiseOnRequestFailure = options.raiseOnRequestFailure
    ...
    if (urlOrHtml.match(/^http/i)) {
      ...
      const displayUrl = options.displayUrl
      delete options.displayUrl
      await page.goto(displayUrl || "http://example.com", requestOptions)
    }

    // add styles (if provided)
    const styleTagOptions = options.styleTagOptions
    delete options.styleTagOptions
    ...
    const scriptTagOptions = options.scriptTagOptions
    delete options.scriptTagOptions
    ...
    delete options.waitForSelector
    const waitForSelectorOptions = options.waitForSelectorOptions
    ...
    const waitForTimeout = options.waitForTimeout
    delete options.waitForTimeout
    ...
    const hoverSelector = options.hover
    ...
    if (
      debug === undefined ||
      (typeof debug === "object" &&
        (debug.headless === undefined || debug.headless))
    ) {
...
function _handleError(error) {
  if (error instanceof Error) {
    process.stdout.write(
      JSON.stringify([
        "err",
        error.toString().replace(new RegExp("^" + error.name + ": "), ""),
        error.name
      ])
    )
  } else {
