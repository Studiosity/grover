...
const _processPage = (async (convertAction, urlOrHtml, options) => {
  ...
  try {
    ...
    const addStyleTag = options.addStyleTag; delete options.addStyleTag;
    if (addStyleTag !== undefined) {
      await page.addStyleTag(addStyleTag);
    }

    // add scripts (if provided)
    const addScriptTag = options.addScriptTag; delete options.addScriptTag;
    if (addScriptTag !== undefined) {
      await page.addScriptTag(addScriptTag);
    }
