...
const _processPage = (async (convertAction, uriOrHtml, options) => {
  ...
  try {
    ...
    // If specified (and enabled) and the browser is Chrome v131+ (supports the permission) allow local network access
    const allowLocalNetworkAccess = options.allowLocalNetworkAccess; delete options.allowLocalNetworkAccess;
    const browserVersion = await browser.version();
    if (allowLocalNetworkAccess === true && Number.parseInt(browserVersion.match('^Chrome/(\\d+)')?.[1]) >= 131) {
