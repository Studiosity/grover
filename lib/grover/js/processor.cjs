...
function _handleError(error) {
  if (error instanceof GroverError) {
    process.stdout.write(JSON.stringify(['err', error.message, error.name, error.errors, error.stack]));
  } else if (error instanceof Error) {
    process.stdout.write(
      JSON.stringify(['err', error.toString().replace(new RegExp('^' + error.name + ': '), ''), error.name, null, error.stack])
    );
  } else {
    process.stdout.write(JSON.stringify(['err', error.toString(), null, null, error.stack]));
  }
  process.stdout.write("\n");
}
